...
Real pipeline (replaces send_xy_coords_loop.py's random values):
    ...
    - Every 4 seconds, picks the NEXT detected object in round-robin order
      and sends its (x, y) to the real JAKA arm
...
class DetectedCoordSender(Node):
    def __init__(self):
        ...
        service_name = '/jaka_driver/joint_move'
        self.get_logger().info(f'Connecting to service: {service_name}')
        ...
        while not self._client.wait_for_service(timeout_sec=1.0):
            self.get_logger().info('Waiting for JAKA joint_move service...')
    ...
    def send_target(self, joint_positions, duration_sec):
        ...
        if not self._client.service_is_ready():
            self.get_logger().error(
                'JAKA joint_move service not available'
            )
            return False

        request = Move.Request()

        request.pose = [float(x) for x in joint_positions]
        ...
        self.get_logger().info(
            f'Sending target joint positions: {joint_positions}'
        )
    ...
    def on_service_response(self, future):
        ...
        try:
            ...
            self.get_logger().info(
                'JAKA movement command completed.'
            )

        except Exception as e:
            self.get_logger().error(
                f'JAKA service failed: {e}'
            )
